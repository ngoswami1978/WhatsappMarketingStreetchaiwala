    ...
    public class Messegner
    {
		...
		public void SendMessage(string number, string message, uint load_timeout = 30, uint ticks_timeout = 10, uint wait_after_send=2)
        {
            try
            {
                ...
                // ➊ Jump straight to that chat inside the *already-open* tab
                var url =
                  $"https://web.whatsapp.com/send?phone={number}&text={HttpUtility.UrlEncode(message)}&type=phone_number&app_absent=0";
        ...
        public void SendMedia(MediaType mediaType, string number, string path, string caption=null, uint load_timeout = 30, uint ticks_timeout = 20)
        {
            ...
            {                
                ...
                WaitForCSSElemnt("[aria-placeholder=\"Type a message\"]", load_timeout);
                ...
                var textbox = WaitForCSSElemnt("div[aria-label='Send'][role='button']");
                Wait(3);
        ...
        private IWebElement WaitForCSSElemnt(string selector, uint timeoutInSeconds = 5)
        {
            ...
            {
                Console.WriteLine($"[Timeout] Could not find element with selector: {selector}");
                return null;
            }
            catch (NoSuchElementException)
            {
                Console.WriteLine($"[Error] Element not found for selector: {selector}");
                return null;
            }
            catch (Exception ex)
            {
